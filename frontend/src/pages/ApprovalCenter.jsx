...
export default function ApprovalCenter() {
  ...
  const [view, setView] = useState("active"); // active | pending | needs_info | approved | rejected | ignored | all
  ...
  const fetchData = async () => {
    try {
      ...
      const [
        ...
        axios.get("/api/approvals/review-candidates/?scope=history"),
      ]);
  ...
  const filteredApprovals = useMemo(() => {
    ...
    return approvals.filter((item) => {
      ...
      const matchesView =
        view === "all" ||
        (view === "active" &&
          ACTIVE_APPROVAL_STATUSES.includes(item.status)) ||
        (view === "pending" && item.status === "pending") ||
        (view === "needs_info" && item.status === "needs_info") ||
  ...
  const activeCount = approvals.filter((a) =>
    ACTIVE_APPROVAL_STATUSES.includes(a.status)
  ).length;
  const pendingCount = approvals.filter((a) => a.status === "pending").length;
  const needsInfoCount = approvals.filter((a) => a.status === "needs_info").length;
          ...
          {[
            {
              ...
              value:
                activeCount,
              description:
                "Requests waiting for a decision or more information.",
              style:
              ...
              <div className="divide-y divide-slate-100">
                ...
                {filteredApprovals.map(
                  (item) => {
                    ...
                    const actionable =
                      ACTIVE_APPROVAL_STATUSES.includes(
                        item.status
                      );
        ...
        <section className="mt-4 overflow-hidden rounded-[24px] border border-slate-200 bg-white shadow-sm">
          <div className="flex flex-col gap-2 border-b border-slate-100 px-5 py-4 sm:flex-row sm:items-center sm:justify-between sm:px-6">
            <div>
              ...
              <p className="mt-1 text-xs leading-5 text-slate-500">
                Promoted and rejected suggestions remain traceable. Same-thread repeats become history rather than duplicate approvals.
              </p>
            </div>
              ...
              {reviewHistory.slice(0, 12).map((candidate) => (
                <article key={candidate.id} className="px-5 py-4 sm:px-6">
                  ...
                  <h3 className="mt-2 text-sm font-semibold text-slate-900">
                    {candidate.title || "Resolved Approval suggestion"}
                  </h3>
                    ...
                    <div className="mt-3 grid gap-2 md:grid-cols-2 xl:grid-cols-3">
                      {candidate.history.slice(0, 3).map((occurrence) => (
                        <button
                          ...
                          onClick={() => window.location.assign(occurrence.open_url || "/inbox")}
