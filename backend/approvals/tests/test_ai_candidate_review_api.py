...
class AIApprovalCandidateReviewAPITests(TestCase):

    def setUp(self):
        self.user = User.objects.create_user(
            email="review-approval@test.com",
            password="pass123",
        )

        self.organization = Organization.objects.create(
            name="Review Approval Org",
            slug="review-approval-org",
        )
        ...
        self.message = InboxMessage.objects.create(
            ...
            platform="gmail",
            direction="inbound",
            external_message_id="review-approval-message-1",
        ...
        self.candidate = AIApprovalCandidate.objects.create(
            user=self.user,
            organization=self.organization,
            message=self.message,
            title="Approve production deployment",
            description="Authorize production deployment.",
            confidence_score=90,
            priority=90,
            evidence="Please approve production deployment.",
            reason="Explicit authorization requested.",
            provider="openai",
            model="gpt-5.6-luna",
        )
    ...
    def test_promote_candidate_creates_approval_and_evidence(self):
    ...
    def test_reject_candidate_creates_no_approval(self):
        response = self.client.post(
            (
                "/api/approvals/ai-candidates/"
    ...
    def test_cross_tenant_candidate_returns_404(self):
        ...
        other_message = InboxMessage.objects.create(
            ...
            platform="gmail",
            direction="inbound",
            external_message_id="other-approval-message",
        ...
        other_candidate = AIApprovalCandidate.objects.create(
            user=other_user,
            organization=other_org,
            message=other_message,
            title="Other approval",
            confidence_score=90,
            evidence="Please approve this.",
        )
